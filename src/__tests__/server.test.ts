...
describe('MCP Server', () => {
  ...
  test('server has a name and version', () => {
    const server = createMcpServer();
    // The server object from MCP SDK has these properties
  ...
  test('list-citations tool returns empty list', async () => {
    ...
    // Simulate the CallTool request by calling the handler directly
    // We test the tool logic by invoking the registered handler
  ...
  test('get-citation tool returns not found message', async () => {
    ...
    const result = await (server as unknown as {
      ...
      .get('tools/call')!({
        ...
        params: {
          name: 'get-citation',
          arguments: { doi: '10.0000/not-found' },
        },
      });
    ...
    const result = await (server as unknown as {
      ...
      .get('tools/call')!({
        ...
        params: {
          ...
          arguments: { doi: '10.1234/test', score: 0.9, notes: 'good paper' },
        },
      });

    const content = (result as { content: Array<{ type: string; text: string }> }).content;
    expect(content[0].text).toContain('0.9');
    expect(db.updateTrustScore).toHaveBeenCalledWith(
      '10.1234/test',
      0.9,
      'good paper',
      undefined
