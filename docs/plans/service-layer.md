...
- `src/services/` exists: `contracts.ts` (shared zod contracts +
  ...
  `verify-quote` in `src/mcp/tools/grounding.ts`) derive their `inputSchema`
  from the contracts.
- Eight MCP tools total. The five older ones (`get-citation`,
  `list-citations`, `import-bibtex`, `search-arxiv` in
  `src/mcp/tools/citations.ts`; `download-pdf` in `src/mcp/tools/retrieval.ts`)
...
| read-content        | ContentService.read (shipped)  | `read-content`                  | none                      | `GET /api/v1/citations/{doi}/content` |
| verify-quote        | see fts5 doc (shipped)         | `verify-quote`                  | none                      | deferred                              |
| get-citation        | CitationService.get (expl.)    | `get-citation`                  | none                      | `GET /api/v1/citations/{doi}`         |
...
| search-arxiv        | thin resolver call             | `search-arxiv`                  | none                      | deferred                              |
| get-references      | GraphService (expl.)           | `get-references`                | none                      | deferred                              |
| get-citing-papers   | GraphService (expl.)           | `get-citing-papers`             | none                      | deferred                              |
| related-papers      | GraphService (expl.)           | `related-papers`                | none                      | deferred                              |
| check-corpus        | GraphService (expl.)           | `check-corpus`                  | none                      | deferred                              |
| expand-corpus       | GraphService (expl.)           | `expand-corpus`                 | none                      | deferred                              |
